...
def _generate_one_reflection_task(person_item: dict, combined_content: str, gemini_client) -> tuple:
    """한 명의 현시대 반영 생성 (키워드 약 50개). ThreadPoolExecutor용. 반환: (record_id, row_idx, text). 실패 시 text=''."""
    ...
    if persona and persona not in ("", "nan"):
        profile_parts.append(f"페르소나: {persona[:300]}")
    profile_str = "\n".join(profile_parts) if profile_parts else "특성 없음"
    prompt = f"""다음은 '현시대 자료'입니다 (첨부 글/파일 내용).
---
{combined_content[:12000]}
---
위 현시대 자료와 아래 가상인물 정보를 바탕으로, **당신이 이해한 이 인물의 관심분야**를 키워드로만 반영하세요.
- 문장이나 설명 없이 **키워드만** 약 50개 생성하세요. (쉼표 또는 공백으로 구분)
- 자료에서 추출한 키워드 + 이 인물의 페르소나·통계를 '관심분야'로 해석하여 그에 맞는 현시대 관련 키워드를 포함하세요.
- 글자 수 제한 없이 50개 키워드를 모두 나열하세요.

가상인물 정보:
{profile_str}

출력: 키워드만 쉼표로 구분하여 한 줄로 (약 50개, 예: 키워드1, 키워드2, 키워드3, ...):"""
    for attempt in range(LEARN_RETRY_COUNT + 1):
        try:
            ...
            if ":" in text and ("키워드" in text or "출력" in text):
...
def page_virtual_population_db():
    ...
    @st.fragment
    def render_chat_interface(selected_sido_code: str):
        # 대화 종료 버튼 (채팅창 상단)
        if st.button("❌ 대화 종료", key="vdb_chat_close", type="secondary", use_container_width=True):
            st.session_state.show_vdb_chat = False
            st.rerun()
        ...
        def send_message(msg_text: str, response_placeholder=None):
            ph = response_placeholder  # 고정 위치 placeholder (로딩 → 스트리밍 덮어쓰기용)
                    ...
                    if selected_person is None:
                        ...
                        if ph:
                            ...
                            <div class="chat-message-wrapper assistant-msg">
                                <span class="chat-label">안내</span>
                                <div class="chat-bubble assistant-bubble">가상인구를 선택해주세요.</div>
                            </div>
                            """, unsafe_allow_html=True)
                    else:
        ...
        사용자 질문: {user_input}"""
                    ...
                    if not db_rows:
                        ...
                        if ph:
                            ...
                            <div class="chat-message-wrapper assistant-msg">
                                <span class="chat-label">안내</span>
                                <div class="chat-bubble assistant-bubble">가상인구 DB에 데이터가 없습니다. 상단에서 **전체 데이터 불러오기**를 누른 뒤 다시 시도해주세요.</div>
                            </div>
                            """, unsafe_allow_html=True)
                    else:
                        all_dfs = []
        ...
        사용자 질문: {user_input}"""
                            ...
                            if ph:
                                ...
                                <div class="chat-message-wrapper assistant-msg">
                                    <span class="chat-label">안내</span>
                                    <div class="chat-bubble assistant-bubble">가상인구 데이터를 불러올 수 없습니다.</div>
                                </div>
                                """, unsafe_allow_html=True)
                ...
                if ph:
                    import html
                    escaped = html.escape(response_text).replace('\n', '<br>')
                    ph.markdown(f"""
                    <div class="chat-message-wrapper assistant-msg">
                        <span class="chat-label">안내</span>
                        <div class="chat-bubble assistant-bubble">{escaped}</div>
                    </div>
                    """, unsafe_allow_html=True)
            if response_text:
                ...
                if st.session_state.chat_mode == "1:1대화":
                    ...
                    if selected_person is not None:
                        chat_entry["person_info"] = selected_person.to_dict() if hasattr(selected_person, 'to_dict') else dict(selected_person)
                elif st.session_state.chat_mode == "5:1대화":
                    selected_people = st.session_state.get("selected_chat_people")
        ...
        if not st.session_state.chat_history:
            ...
            for chat in st.session_state.chat_history:
                escaped_message = html.escape(str(chat['message'])).replace('\n', '<br>')
                if chat["role"] == "user":
                    st.markdown(f"""
        ...
        # Placeholder 패턴: 입력창 바로 위 한 칸에 로딩 → 스트리밍으로 덮어쓰기
        if st.session_state.get("chat_loading") and st.session_state.chat_history and st.session_state.chat_history[-1].get("role") == "user":
            ...
            """, unsafe_allow_html=True)
            send_message(st.session_state.chat_history[-1]["message"], response_placeholder)
    ...
    def _vdb_frag():
        ...
        with col_db:
            ...
            # 표시: 미리보기 20명만 표시 (전체 표시 시 인원 많을 때 느려짐), 다운로드는 전체
            if combined_df is not None:
                table_container = st.container()
                with table_container:
                    preview_limit = 20
                    preview_df = combined_df.head(preview_limit)
                    st.caption(f"미리보기 (상위 {preview_limit}명) — 전체 {total_count:,}명 중 표시. CSV 다운로드/업로드·DB 변경은 전체 데이터 기준.")
                    ...
                    with col_dl2:
                        st.caption("CSV 업로드")
                        csv_upload = st.file_uploader("CSV 파일 선택", type=["csv"], key="vdb_csv_upload", label_visibility="collapsed")
                        if csv_upload is not None:
                            if st.button("DB 수정 (업로드 파일로 반영)", type="primary", key="vdb_apply_csv"):
                                try:
                                    up_df = pd.read_csv(csv_upload, encoding="utf-8-sig")
                                    if up_df.empty:
                                        st.error("업로드한 CSV에 데이터가 없습니다.")
                                    else:
                                        if "페르소나" not in up_df.columns:
                                            up_df["페르소나"] = ""
                                        if "현시대 반영" not in up_df.columns:
                                            up_df["현시대 반영"] = ""
                                        data_json = up_df.to_json(orient="records", force_ascii=False)
                                        ...
                                        first_id = insert_virtual_population_db_chunked(
                                            selected_sido_code, selected_sido_name, ts, "csv_upload.csv", data_json
                                        )
                                        if first_id is not None:
                                            ...
                                            st.success(f"DB를 업로드 파일로 수정했습니다. (총 {len(up_df):,}명)")
                                            st.rerun()
                                        else:
                                            st.error("DB 반영 중 오류가 발생했습니다.")
                                except Exception as e:
                                    st.error(f"CSV 처리 실패: {e}")
                        else:
                            st.caption("CSV 업로드 후 [DB 수정]")
                    with col_dl3:
                        if st.button("DB 변경", type="primary", use_container_width=True, key="vdb_sync_all_btn"):
                            to_sync = st.session_state.get(f"vdb_cached_jsons_{selected_sido_code}") or []
                            if not to_sync:
                                st.warning("반영할 캐시 데이터가 없습니다. 먼저 전체 데이터를 불러오세요.")
                            else:
                                sync_bar = None
                                try:
                                    sync_bar = st.progress(0.0, text="서버에 전체 변경 저장 중…")
                                    ok = 0
                                    for i, (rid, data_json) in enumerate(to_sync):
                                        try:
                                            if update_virtual_population_db_record_persona_reflection_only(rid, data_json):
                                        ...
                                        sync_bar.progress((i + 1) / len(to_sync), text=f"저장 중… {i+1}/{len(to_sync)}")
                                    sync_bar.empty()
                                    st.success(f"가상인구 DB 전체 {len(to_sync)}건을 서버에 저장했습니다.")
                                except Exception as e:
                                    if sync_bar:
                                        try:
                                            sync_bar.empty()
        ...
        with col_dialog:
            ...
            if st.session_state.chat_mode == "전체 학습":
                ...
                st.caption("텍스트를 입력하거나 PDF를 업로드하면, Gemini가 각 가상인구의 관심분야를 자기 방식으로 해석하여 '현시대 반영'을 **키워드 약 50개** 형태로 생성·대입합니다. 글자 수 제한 없이 50개 키워드가 모두 저장됩니다.")
                ...
                with col_btn2:
                    st.caption("실행 시 가상인구 DB의 '현시대 반영' 컬럼이 키워드(약 50개)로 갱신됩니다. (페르소나가 있는 가상인구만)")
                ...
                if run_learn_btn:
                    ...
                    if not combined_content:
                        ...
                        db_rows = st.session_state.get(f"vdb_cached_jsons_{selected_sido_code}")
                        if not db_rows:
                            st.warning("가상인구 DB에 데이터가 없습니다. **전체 데이터 불러오기**를 먼저 실행해주세요.")
                        else:
                            work_queue = []
                            for record_id, data_json in db_rows:
                                df = pd.read_json(data_json, orient="records")
                                if "현시대 반영" not in df.columns:
                                    df["현시대 반영"] = ""
                                if "페르소나" not in df.columns:
                                ...
                                for idx, row in df.iterrows():
                                    ...
                                    if persona and persona not in ("", "nan") and (not reflection or reflection == "" or reflection == "nan"):
                                        work_queue.append({"record_id": record_id, "row_idx": int(idx), "row": row.to_dict() if hasattr(row, "to_dict") else dict(row)})
                            if not work_queue:
                                st.info("현시대 반영이 비어 있는 가상인구가 없습니다. (페르소나가 있고 현시대 반영이 비어 있는 인원만 대상)")
                            else:
                                ...
                                st.session_state.learn_total_done = 0
                                st.session_state.learn_work_queue = work_queue
                                st.session_state.learn_next_index = 0
                                st.rerun()
            
                # 전체 학습 실행 중 — work_queue 기준 청크 단위 처리, 중지 전까지 반복
                if st.session_state.learn_running and not st.session_state.learn_stop:
                    ...
                    work_queue = st.session_state.get("learn_work_queue") or []
                    next_index = st.session_state.get("learn_next_index", 0)
                    total_work = len(work_queue)
                    if not combined_content or total_work == 0:
                        if total_work == 0:
                        ...
                        st.session_state.learn_running = False
                        st.session_state.learn_stop = False
                        for k in ("learn_combined_content", "learn_sido_code", "learn_total_done", "learn_work_queue", "learn_next_index"):
                            st.session_state.pop(k, None)
                        st.rerun()
                    elif next_index >= total_work:
                        total_done = st.session_state.get("learn_total_done", 0)
                        st.success(f"전체 학습 완료: 총 {total_done}명의 가상인구에 현시대 반영을 반영했습니다.")
                        st.session_state.learn_running = False
                        st.session_state.learn_stop = False
                        ...
                        try:
                            cache_key = f"vdb_cached_jsons_{run_sido}"
                            cached_list = st.session_state.get(cache_key) or []
                            id_to_json = {rid: js for (rid, js) in cached_list}
                            batch_end = min(next_index + LEARN_CHUNK_SIZE, total_work)
                            chunk = [work_queue[i] for i in range(next_index, batch_end)]
                            progress_bar = st.progress(next_index / total_work if total_work else 0)
                            status_placeholder = st.empty()
                            total_done = st.session_state.get("learn_total_done", 0)
                            status_placeholder.caption(f"처리 중: {next_index + 1}~{batch_end} / {total_work}명 (누적 {total_done}명 완료)")
                            gemini_client = GeminiClient()
                            person_items = [{"record_id": item["record_id"], "row_idx": item["row_idx"], "row": item["row"]} for item in chunk]
                            results_by_record = {}
                            with ThreadPoolExecutor(max_workers=LEARN_MAX_WORKERS) as executor:
                                futures = {executor.submit(_generate_one_reflection_task, p, combined_content, gemini_client): p for p in person_items}
                                completed = 0
                                for fut in as_completed(futures):
                                    if st.session_state.get("learn_stop"):
                                        break
                                    try:
                                        record_id, row_idx, text = fut.result()
                                        results_by_record.setdefault(record_id, []).append((row_idx, text))
                                    except Exception:
                                        pass
                                    completed += 1
                                    status_placeholder.caption(f"처리 중: {next_index + completed}/{batch_end} (전체 {total_work}명 중)")
                                    progress_bar.progress((next_index + completed) / total_work)
                            for record_id, updates in results_by_record.items():
                                data_json = id_to_json.get(record_id)
                                if not data_json:
                                    continue
                                df = pd.read_json(data_json, orient="records")
                                if "현시대 반영" not in df.columns:
                                ...
                                id_to_json[record_id] = new_json
                            processed_this_run = len(chunk)
                            progress_bar.empty()
                            status_placeholder.empty()
                            st.session_state.learn_total_done = total_done + processed_this_run
                            st.session_state.learn_next_index = batch_end
                            if st.session_state.get("learn_stop"):
                                st.warning(f"전체 학습이 중지되었습니다. 지금까지 {st.session_state.learn_total_done}명에 현시대 반영을 반영했습니다.")
                                st.session_state.learn_running = False
                                st.session_state.learn_stop = False
                                for k in ("learn_combined_content", "learn_sido_code", "learn_total_done", "learn_work_queue", "learn_next_index"):
                                    st.session_state.pop(k, None)
        ...
        with col_chat:
            st.subheader("채팅창")
            # 버튼식 활성화(Lazy Loading): 채팅 활성화 시에만 무거운 채팅 로직 실행
            if "show_vdb_chat" not in st.session_state:
                st.session_state.show_vdb_chat = False
            if st.session_state.chat_mode == "전체 학습":
                st.info("전체 학습 모드에서는 채팅 기능을 사용할 수 없습니다. 1:1대화 또는 5:1대화 모드를 선택해주세요.")
            elif not st.session_state.show_vdb_chat:
                if st.button("💬 가상인구와 대화 시작하기", type="primary", key="vdb_chat_start", use_container_width=True):
                    st.session_state.show_vdb_chat = True
                    st.rerun()
            else:
                render_chat_interface(selected_sido_code)
    _vdb_frag()
