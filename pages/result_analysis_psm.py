...
def page_psm():
    ...
    if records:
        ...
        use_real_data_option = st.checkbox("2차 대입 결과 데이터 사용", value=False, key="psm_use_real_data")
        
        if use_real_data_option:
            selected_label = st.selectbox(
                "2차 대입 결과에서 가상인구 데이터를 선택하세요:",
                options=list(record_options.keys()),
                index=0 if not st.session_state.get("psm_selected_record_label") else 
                      (list(record_options.keys()).index(st.session_state.psm_selected_record_label) 
                       if st.session_state.psm_selected_record_label in record_options else 0),
                key="psm_record_select"
            )
            ...
            if selected_label and selected_label in record_options:
                ...
                if excel_path and os.path.isfile(excel_path):
                    try:
                        ...
                        st.session_state.psm_population_df = real_data_df
                        st.session_state.psm_selected_record_label = selected_label
                        use_real_data = True
            ...
            st.session_state.psm_population_df = None
